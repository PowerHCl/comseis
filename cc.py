...
st1=read(f1)[0]
st2=read(f2)[0]
d1=st1.stats.sac.delta
d2=st2.stats.sac.delta
d1=round(d1,3)
d2=round(d2,3)
d=d1
...
if t=='t0':
    ...
    exit()
st11=st1.trim(ts1+tp1-b1,ts1+tp1+s-b1)
st22=st2.trim(ts2+tp2-b2,ts2+tp2+s-b2)

if d2>d1:
    st22=st22.interpolate(sampling_rate=1/d1)
    d=d1
elif d1>d2:
    st11=st11.interpolate(sampling_rate=1/d2)
    d=d2
...
if value < 0.5:
    value,tlag,timelag=cc(st11.data,st22.data,maxlagn*2)
    if value<0.7:
        value,tlag,timelag=cc(st11.data,st22.data,maxlagn*3)
    if value >=0.8:
        if timelag>=0:
            st22=st2.trim(ts2+tp2-b2+timelag*d,ts2+tp2+s-b2+timelag*d)
        else:
            st11=st1.trim(ts1+tp1-b1+timelag*d,ts1+tp1+s-b1+timelag*d)
        value,tlag,timelag=cc(st11.data,st22.data,maxlagn)
elif value>0.8 and value <0.9 and timelag*d >=1:
    if timelag>=0:
        st22=st2.trim(ts2+tp2-b2+timelag*d,ts2+tp2+s-b2+timelag*d)
    else:
        st11=st1.trim(ts1+tp1-b1+timelag*d,ts1+tp1+s-b1+timelag*d)
